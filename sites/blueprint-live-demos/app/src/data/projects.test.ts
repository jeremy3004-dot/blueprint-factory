...
test("live demo registry exposes all eleven approved public builds", () => {
  assert.equal(liveDemoCount, 11);
  assert.equal(liveDemoRecords.length, 11);
});
...
test("carousel and index CTAs point at the expected external demo URLs", () => {
  const expected = {
    ...
    "gurkha-fit": "https://gurkhafit.app"
  };
