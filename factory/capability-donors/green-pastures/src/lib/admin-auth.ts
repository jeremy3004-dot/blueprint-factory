...
const ADMIN_SESSION_COOKIE = "gptrek_admin_session";
const ADMIN_SESSION_TTL_SECONDS = 60 * 60 * 12;

function resolveSessionSecret() {
  return process.env.ADMIN_SESSION_SECRET || process.env.OPS_API_TOKEN || "";
}
...
async function verifyCloudflareAccessJwt(token: string): Promise<AccessJwtPayload | null> {
  ...
  const valid = await crypto.subtle.verify("RSASSA-PKCS1-v1_5", cryptoKey, signature, signedPayload);
...
export async function getAdminIdentityFromHeaders(headers: Headers): Promise<AdminIdentity | null> {
  ...
  if (process.env.ADMIN_AUTH_DISABLED === "true") {
    const email = process.env.ADMIN_DEV_EMAIL ?? "admin@gptrek.local";
  ...
  if (process.env.NODE_ENV !== "production") {
    const email = process.env.ADMIN_DEV_EMAIL ?? "local-admin@gptrek.dev";
