# Codex Execution Prompt — Blueprint Factory Upgrade

Copy everything below the line into Codex, from the Blueprint Factory repo root.
...
4. **Test against reality.** For Phase 1, validate capture against at least two real external sites,
   ...
   section of an existing site and showing the report catches it. Use `sites/example-site` as the
   sandbox for pipeline testing; do not rewrite the other sites' content.
...
9. **If a phase wanders** (you notice you're improvising away from the master plan), stop, write a
   ...
   this factory's own Alpine Bloom precedent.
10. **Ask before:** production deploys, paid services, deleting anything, or changing the factory's
    process documents beyond what the master plan specifies.
